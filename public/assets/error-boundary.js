...
(function(){
  ...
  function showFallbackCard(containerId, title, msg, retryFn){
    ...
    div.innerHTML='<div style="font-weight:900; font-size:13px; margin-bottom:4px;">'+title+'</div><div style="opacity:.9; margin-bottom:8px;">'+msg+'</div><div style="display:flex; gap:8px; flex-wrap:wrap"><button id="vh-retry-'+containerId+'" style="min-height:44px; border:2.2px solid #1A150F; background:#F0E442; border-radius:999px; font-weight:900; padding:0 14px; cursor:pointer; box-shadow:2px 2px 0 #1A150F;">Retry (1s/2s/4s)</button><a href="/offline" style="min-height:44px; display:inline-flex; align-items:center; border:2.2px solid #1A150F; background:#fff; color:#1A150F; border-radius:999px; padding:0 14px; font-weight:900; text-decoration:none; box-shadow:2px 2px 0 #1A150F;">Offline mode →</a></div>';
  ...
  function handleVHCustomFailures(){
    window.addEventListener('vh:mtnn-failed', function(){
      showFallbackCard('daily-guesses','Embeddings failed to load','64-d MTNN 3.2MB embeddings took too long. Daily cosine still works offline from cache if previously visited. Retrying 1s/2s/4s.', function(){
