...
sys.path.insert(
    0, str(Path(__file__).parent.parent.parent / "dottie" / "apps" / "scout-cli")
)
try:
    from bigbang.plugins.vector.operator import (
        ...
        train_smoke,
        write_triple_checkpoint,  # noqa: F401 — imported to assert the operator API surface
    )
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="vector-hoops MLOps operator (scout/mlops-operator lane3)"
    )
