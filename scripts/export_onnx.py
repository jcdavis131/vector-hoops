...
def export_mock_onnx(out_path: Path, config: dict):
    ...
    try:
        ...
        model = MTNN(fam_dims, n_seasons=30, d_tower=32, d_tower_hidden=160, d_emb=48, 
                     ...
                     mlp_heads=True, d_head_hidden=128, d_model=96)
        model.eval()
        dummy_xs = {f"fam{i}": torch.randn(1, 7) for i in range(17)}
        dummy_ms = {f"fam{i}": torch.ones(1, 7) for i in range(17)}
        ...
        class Wrapper(torch.nn.Module):
            ...
            def forward(self, *tower_inputs):
                ...
                parts = torch.stack([self.mtnn.towers[fam](xs[fam], ms[fam]) for fam in self.mtnn.families], dim=1)
                emb = self.mtnn.fusion(parts, season)
                return emb
        
        wrapper = Wrapper(model)
        dummy_towers = tuple(torch.randn(1, 7) for _ in range(17))
        torch.onnx.export(wrapper, dummy_towers, str(out_path), 
...
def check_bundle_size():
    ...
    if mtnn_js.exists():
        print(f"  mtnn.js: {mtnn_js.stat().st_size/1024:.1f} KB raw ({mtnn_js.stat().st_size/1024*0.3:.1f} KB est gz)")
