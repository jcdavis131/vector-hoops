...
def test_ablate_v5_basic(input_val, expected, tmp_path):
    ...
    if target_module is None:
        pytest.skip("pipeline.ablate_v5 not importable in test env")
...
def test_ablate_v5_invalid_inputs(bad_input, tmp_path):
    ...
    if target_module is None:
        pytest.skip("pipeline.ablate_v5 not importable")
...
def test_ablate_v5_integration(sample_data, tmp_path):
    ...
    tmp_file = tmp_path / "ablate_v5_sample.json"
...
def test_ablate_v5_file_io(tmp_path):
    ...
    pytest.skip(
        "TODO: wire file IO into actual ablate_v5 logic – stub intentionally incomplete"
    )
