"""real tests for pipeline.ablate_v5 - wired from coverage gap mapper"""
...
spec = importlib.util.spec_from_file_location(f"pipeline.ablate_v5", str(MOD_PATH))
...
def test_module_callables_exist():
    ...
    for name in ['resolve_device', 'train_one', '_rmse', 'verdict']:
        assert hasattr(mod, name)



def test_tmp_path_integration(tmp_path):
    sample = {"module": "ablate_v5", "input": 1, "season": "2023-24"}
    p = tmp_path / f"ablate_v5.json"
