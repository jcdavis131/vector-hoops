...
spec = importlib.util.spec_from_file_location(f"pipeline.build_skills", str(MOD_PATH))
...
def test_weight_matrix():
    ...
    feats = ["PTS","AST","OREB","DREB","STL","BLK","TOV","FG3A","FGA","FTA","FG3_PCT","FG_PCT","FT_PCT","PLUS_MINUS"]
...
def test_tmp_path_integration(tmp_path):
    ...
    p = tmp_path / f"build_skills.json"
