"""real tests for pipeline.build_wide_skills - wired from coverage gap mapper"""
...
PIPE = ROOT / "pipeline"
MOD_PATH = PIPE / "build_wide_skills.py"
...
spec = importlib.util.spec_from_file_location(f"pipeline.build_wide_skills", str(MOD_PATH))
...
def test_module_callables_exist():
    ...
    for name in ['norm_name', 'load_caches', 'zscore', '_configure_stdio']:
...
def test_tmp_path_integration(tmp_path):
    sample = {"module": "build_wide_skills", "input": 1, "season": "2023-24"}
    p = tmp_path / f"build_wide_skills.json"
    p.write_text(json.dumps(sample))
    assert p.exists()
    data = json.loads(p.read_text())
    assert data["module"] == "build_wide_skills"
