"""real tests for pipeline.compute_eligibility_gates - wired from coverage gap mapper"""
...
PIPE = ROOT / "pipeline"
MOD_PATH = PIPE / "compute_eligibility_gates.py"
...
spec = importlib.util.spec_from_file_location(f"pipeline.compute_eligibility_gates", str(MOD_PATH))
...
def test_module_callables_exist():
    ...
    for name in ['load_gamelog_seasons', 'eligible', 'pctile', 'main']:
        assert hasattr(mod, name)



def test_tmp_path_integration(tmp_path):
    sample = {"module": "compute_eligibility_gates", "input": 1, "season": "2023-24"}
    p = tmp_path / f"compute_eligibility_gates.json"
    p.write_text(json.dumps(sample))
    assert p.exists()
    data = json.loads(p.read_text())
    assert data["module"] == "compute_eligibility_gates"
