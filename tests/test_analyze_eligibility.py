"""real tests for pipeline.analyze_eligibility - wired from coverage gap mapper"""
...
PIPE = ROOT / "pipeline"
MOD_PATH = PIPE / "analyze_eligibility.py"
...
spec = importlib.util.spec_from_file_location(f"pipeline.analyze_eligibility", str(MOD_PATH))
...
def test_tmp_path_integration(tmp_path):
    sample = {"module": "analyze_eligibility", "input": 1, "season": "2023-24"}
    p = tmp_path / f"analyze_eligibility.json"
