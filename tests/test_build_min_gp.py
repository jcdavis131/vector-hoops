"""real tests for pipeline.build_min_gp - wired from coverage gap mapper"""
...
spec = importlib.util.spec_from_file_location(f"pipeline.build_min_gp", str(MOD_PATH))
...
def test_module_callables_exist():
    ...
    for name in ['from_gamelogs', 'from_api', 'main']:
        assert hasattr(mod, name)



def test_tmp_path_integration(tmp_path):
    sample = {"module": "build_min_gp", "input": 1, "season": "2023-24"}
    p = tmp_path / f"build_min_gp.json"
    p.write_text(json.dumps(sample))
    assert p.exists()
    data = json.loads(p.read_text())
    assert data["module"] == "build_min_gp"
