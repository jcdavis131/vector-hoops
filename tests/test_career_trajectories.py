...
def _require_target():
    if TARGET is None:
        pytest.skip(
            f"Target module pipeline.career_trajectories not importable: {_IMPORT_ERROR} – TODO: fix import path"
        )
...
def test_career_trajectories_smoke_import():
    ...
    pytest.skip(
        "TODO: enumerate expected API – ['classify', 'skill_arc_summary', 'pick_examples'] []"
    )
...
def test_career_trajectories_classify_contract(sample_data):
    ...
    if not hasattr(TARGET, "classify"):
        pytest.skip("TARGET missing classify – TODO verify name")
    fn = TARGET.classify
