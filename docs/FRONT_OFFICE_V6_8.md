# Front Office v6.8 — Valuations + W/$B Pill

**Date:** 2026-08-09
**Node:** valuation-20260809
**Lane:** v6.8 valuations exposure

## v6.8 Summary
- Method: `FOR.35D+.35C+.30F+ring+valα W*=W+2.5*PO+0.12*realG · W*/$M=W*/Pay · W/$B=W/$Val · Valα ±2 brand lift`
- 21 cols: `# Team FOR G W PO W* Pay24 Pay25 W/$M W*/$M PO/$M $Val W/$B W*/$B PO/$B ValSc Draft Cap Fore Flex`
- Sort pills added: `$Val, ValΔ%, W/$B, W*/$B, PO/$B, ValSc` in addition to W, W*, W/$M, W*/$M, PO/$M
- Valuation synth 360 entries `assets/data/team_valuations.json` retained (2014-15..2025-26 30 teams 12 seasons Forbes synth estimated training only)
- Val scoring: `wins_per_b = W / (valuation_m/1000)` → median ~5.0, `val_score = 50 + (wpb-median)*12 + growth*0.5` clamp 0-100 grade A+..F, `valuation_alpha = clamp((val_score-50)/25*1.5 + growth/20*0.2, -2,2)` → ±2 FOR lift
- Exposure: `front_office.json` now includes `valuations_by_season` (season->{abbr:val_m}), `valuations_meta_by_season`, `valuation_by_season` alias for time-machine fun boards — 12 seasons 360 total
- `teams-board.js` getS() already includes val, val_delta, wpb, weighted_wpb, po_wpb, val_score sorts — PASS
- `teams-time.js` boardHist fallback adds valuation synth (tHistVal fallback $Val B), boardHistFull fields valM2 valGrow2 wpb2 wwpb2 powpb2 vscore2 — PASS
- `teams.html` banner bumped v5.4→v6.8, method pill v6.8, size 14022 (now ~15k) <28672 PASS, 21 cols PASS
- Zero-deps check: bundles/zero_deps.json true, candidate.json 8.9 ≥8.0 PASS, monthly_clean cron exists, exports/ prune rule PASS
- Triple-write: canonical 131 lines bundles/ultra/runs = dottie/pipeline/runs = .scout/missions/_canonical/timeline/timeline.jsonl identical PASS, valuation-20260809 3/3 identical PASS, timeline-fix-20260809 triple PASS
- Live CDN verification required via browser.open ?v=68 — method string present
