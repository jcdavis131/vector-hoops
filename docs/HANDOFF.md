...
/ CI IPs (this environment included). Nothing dormant is faked — the game
surfaces stay hidden until real data lands.

**The one repeating pattern** (copy it for any new source): `fetch_*.py`
(operator-only) → committed `*.example.json` fixture → `build_*.py`
deriver (writes a transparent `assets/*.json` **only from a complete
cache**, plus MTNN labels) → `test_*.py` gates (run against the fixture)
→ wire into `integrate_context.py` / `train_mtnn.py` → `update_dataset.py`
growth-loop step → `operator_fetch_*.sh` runbook → docs.
...
- Commits since master:
  ...
  - `e16aab9` Fix gravity → shooting gravity (Curry) + rim gravity (Wemby)
...
- **Skills Lens** (`skills.html`, `pipeline/build_skills.py`): 12 skills,
  0–99 era-z percentile grades for every player-season →
...
- **MTNN v4** (`pipeline/train_mtnn.py`): per-family towers + a per-skill
  **skill-tower bank** (currently 17 = 12 core + 5 wide, per-skill masked)
  ...
  features; promotion is gated (see SKILLS_LENS §3).
- **Growth loop** (`pipeline/update_dataset.py`): fetch-best-effort →
  rebuild → gate → append `pipeline/cache/dataset_ledger.json`. Weekly
...
Each runbook fetches → rebuilds against real coverage → runs gates →
...
(regenerated by the growth loop / MTNN) — don't rely on it persisting.
