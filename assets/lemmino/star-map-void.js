...
export async function mountStarMap(canvas){
  ...
  try{
    try{
      const j=await cachedFetchJSON('assets/vectors_search_lite_pos.json?v=26');
      rawAll=j.players||[];
    }catch(e){
      const j2=await cachedFetchJSON('assets/vectors_search_lite.json?v=26');
  ...
  try{
    const ts=await cachedFetchJSON('assets/player_team_season.json?v=26').catch(()=>null);
    teamSeasonMap=ts;
  }catch{}
