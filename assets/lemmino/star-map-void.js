...
export async function mountStarMap(canvas){
  ...
  async function cachedFetchJSON(url){
    const CACHE_NAME='vector-hoops-v35-20260721-trading-card';
    try{
      if('caches' in window){
  ...
  try{
    try{
      const j=await cachedFetchJSON('assets/vectors_search_lite_pos.json?v=35');
      rawAll=j.players||[];
    }catch(e){
      const j2=await cachedFetchJSON('assets/vectors_search_lite.json?v=35');
  ...
  try{
    const ts=await cachedFetchJSON('assets/player_team_season.json?v=35').catch(()=>null);
    teamSeasonMap=ts;
  }catch{}
