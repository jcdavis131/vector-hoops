...
class EmbeddingL2(Scene):
    def construct(self):
        ...
        v_norm_out_fg_group, _v_norm_fg = ink_arrow(v_norm_end, OKABE["blue"])
        w_out_fg_group, _w_fg = ink_arrow(w_end, OKABE["sky"])
        ...
        arc = Arc(
            radius=0.62,
            start_angle=w_angle,
            angle=v_angle - w_angle,
            arc_center=sphere_center,
            color=OKABE["yellow"],
            stroke_width=7,
        )
        ...
        self.play(
            ...
            Create(base_circle),
            FadeIn(origin_dot),
            FadeIn(slice_chip, shift=UP * 0.05),
            run_time=0.72,
        )
        # 0.85-1.9
        self.play(
        ...
        self.play(
            Flash(v_norm_end, color=OKABE["blue"], flash_radius=0.24, line_length=0.07),
            run_time=0.18,
        )
        self.wait(0.08)
        ...
        # 3.1-4.2 w + angle
        self.play(
            FadeIn(w_out_fg_group, shift=UP * 0.08),
            FadeIn(w_hat_chip, shift=LEFT * 0.08),
            run_time=0.42,
        )
        ...
        self.play(
            FadeIn(v_dot),
            FadeIn(w_dot),
            FadeIn(final_footer_group, shift=UP * 0.06),
            run_time=0.38,
        )
        self.wait(0.85)
        ...
        self.play(
            ...
            FadeOut(slice_chip),
            FadeOut(v_raw_group),
            FadeOut(v_norm_out_fg_group),
            FadeOut(w_out_fg_group),
            FadeOut(v_hat_chip),
            FadeOut(w_hat_chip),
            FadeOut(arc_group),
            FadeOut(theta_chip),
