...
from cam_style import (
    BG, INK, CARD_FILL, TEXT, SUBTLE_AAA,
    OKABE, TITLE_SIZE, LABEL_SIZE, CODE_SIZE, CAPTION_SIZE,
    INK_STROKE_WIDTH, CORNER_RADIUS,
    apply_cam_style, cam_card,
)
...
class ChimeraEquation(Scene):
    def construct(self):
        ...
        title_card = cam_card(width=5.0, height=0.85, accent_color=OKABE["orange"], corner_radius=CORNER_RADIUS)
        ...
        subtitle = Text("Donor A + Donor B → closest real among 12,392", font_size=16, color=SUBTLE_AAA, font="JetBrains Mono")
        subtitle.next_to(title_group, DOWN, buff=0.15)

        self.play(FadeIn(title_group, shift=DOWN*0.1), FadeIn(subtitle, shift=DOWN*0.06), run_time=0.5)
        self.wait(0.2)

        # Donor A card — smaller to prevent overflow
        donor_a_base = cam_card(width=2.4, height=1.1, accent_color=OKABE["blue"], corner_radius=CORNER_RADIUS)
        ...
        donor_b_base = cam_card(width=2.4, height=1.1, accent_color=OKABE["verm"], corner_radius=CORNER_RADIUS)
        ...
        plus_bg = RoundedRectangle(width=0.45, height=0.45, corner_radius=0.08, fill_color=CARD_FILL, fill_opacity=1, stroke_color=INK, stroke_width=INK_STROKE_WIDTH*0.6)
        ...
        self.play(FadeIn(donor_a, shift=RIGHT*0.1), FadeIn(plus_group), FadeIn(donor_b, shift=LEFT*0.1), run_time=0.5)
        self.wait(0.15)

        # Arrow down to fuse
        arrow1_start = donor_row.get_bottom() + DOWN*0.05
        arrow1_end = arrow1_start + DOWN*0.35
        arrow1 = Arrow(arrow1_start, arrow1_end, color=INK, stroke_width=5, buff=0, max_tip_length_to_length_ratio=0.18)
        
        # Fuse card — compact, centered
        fuse_base = cam_card(width=3.6, height=0.8, accent_color=OKABE["green"], corner_radius=CORNER_RADIUS)
        fuse_title = Text("fuse 48-d", font_size=16, color=TEXT, weight=BOLD)
        fuse_eq = Text("(v_A + v_B)/2 -> v_f L2", font_size=13, color=SUBTLE_AAA, font="JetBrains Mono")
        ...
        arrow2 = Arrow(arrow2_start, arrow2_end, color=INK, stroke_width=5, buff=0, max_tip_length_to_length_ratio=0.18)
        ...
        final_line1 = Text("Chimera(A,B) = argmin r in 12,392", font_size=15, color=TEXT, font="JetBrains Mono", weight=BOLD)
        final_line2 = Text("|| (f(A)+f(B))/2 - f(r) ||_2", font_size=15, color=TEXT, font="JetBrains Mono")
        final_sub = Text("powers daily Chimera game • cosine = similarity", font_size=11, color=SUBTLE_AAA, font="JetBrains Mono")
        ...
        self.play(
            FadeOut(title_group), FadeOut(subtitle),
            FadeOut(donor_row), FadeOut(arrow1), FadeOut(fuse_card),
            FadeOut(arrow2), FadeOut(final_card),
            run_time=0.6
        )
        self.wait(0.5)
