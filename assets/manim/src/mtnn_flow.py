...
def make_mini_tower(abbrev: str, full: str, color: str, icon: str, w: float = 1.18, h: float = 0.60):
    ...
    accent = RoundedRectangle(
        width=w-0.06, height=0.14, corner_radius=0.03,
        fill_color=color, fill_opacity=1, stroke_width=0
    ).move_to(card.get_top()).shift(DOWN*0.10)

    # dot left inside
    dot = Circle(radius=0.07, fill_color=color, fill_opacity=1, stroke_color=INK, stroke_width=2.2)
...
class MTNNFlow(Scene):
    def construct(self):
        ...
        title_txt = Text("Inside MTNN v4 — Cam's Lab", font_size=24, color=INK, font=SANS_STACK[0], weight=BOLD)
        title_txt.move_to(title_card).shift(UP*0.16)
        subtitle_txt = Text("17 families • 120 feats • 48-d L2 • 12,392 seasons • W flow 1380", font_size=13, color=SUBTLE_AAA, font=MONO_STACK[0])
        ...
        t1 = Text("120 feats→17 families", font_size=16, color=INK, font=SANS_STACK[0], weight=BOLD)
        ...
        def mini_box(label):
            b = RoundedRectangle(width=1.5, height=0.32, corner_radius=0.05, fill_color=CARD_FILL, fill_opacity=1, stroke_color=INK, stroke_width=2.5)
        ...
        towers_label = Text("17× residual towers  2 blocks  160→32  LN GELU", font_size=11, color=INK, font=MONO_STACK[0], weight=BOLD)
        towers_label.next_to(towers_grid, UP, buff=0.18)

        self.play(FadeIn(towers_grid, shift=UP*0.08), FadeIn(towers_label, shift=UP*0.04), run_time=0.65)
        self.wait(0.35)
        ...
        d_t1 = Text("TRK tower  13f×2=26 →160", font_size=12, color=INK, font=MONO_STACK[0], weight=BOLD)
        ...
        d_t4 = Text("out 32-d   W tot ~1380 cols", font_size=10, color=INK, font=MONO_STACK[0], weight=BOLD)
        ...
        self.play(towers_grid.animate.set_opacity(0.16), towers_label.animate.set_opacity(0.30), run_time=0.30)
        self.play(FadeIn(detail_group, scale=0.92), run_time=0.5)
        self.wait(0.55)
        self.play(FadeOut(detail_group), towers_grid.animate.set_opacity(1), towers_label.animate.set_opacity(1), run_time=0.40)

        # Arrow to fusion — breathing room
        arrow1 = Arrow(start=towers_grid.get_bottom()+DOWN*0.08, end=towers_grid.get_bottom()+DOWN*0.62, color=INK, stroke_width=4, buff=0.05, max_tip_length_to_length_ratio=0.12)
        ...
        f_t1 = Text("Concat 17×32=544 +12 time =556", font_size=13, color=INK, font=MONO_STACK[0], weight=BOLD)
        f_t2 = Text("556 → 128 GELU LN → 48 → L2 normalize", font_size=11, color=INK, font=MONO_STACK[0])
        f_t3 = Text("v^ = v / ||v||   ||v^||=1   12,392 pts on sphere", font_size=10, color=SUBTLE_AAA, font=MONO_STACK[0])
        ...
        vec = Arrow(sphere_center, sphere_center + RIGHT*0.38 + UP*0.18, buff=0.02, color=OKABE["blue"], stroke_width=4.2, max_tip_length_to_length_ratio=0.18)
        vec_label = Text("48-d v^", font_size=10, color=INK, font=MONO_STACK[0], weight=BOLD).next_to(vec.get_end(), UP+RIGHT, buff=0.04)
        ...
        arrow2 = Arrow(start=fusion_card.get_bottom()+DOWN*0.08, end=fusion_card.get_bottom()+DOWN*0.68, color=INK, stroke_width=4, buff=0.05, max_tip_length_to_length_ratio=0.12)
        ...
        for name, cnt, col, icon in head_defs:
            ...
            ht1 = Text(f"{icon} {name}", font_size=10, color=INK, font=MONO_STACK[0], weight=BOLD)
        ...
        heads_label = Text("Decode heads  CE + MSE  InfoNCE anchor", font_size=10, color=INK, font=MONO_STACK[0])
        ...
        # Footer — truthful
        footer = Text("Flow: 1380 cols • 544+12=556→128→48 L2 • 12,392 careers on sphere • cos=v̂·ŵ", font_size=9, color=SUBTLE_AAA, font=MONO_STACK[0])
