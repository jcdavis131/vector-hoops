...
def main() -> None:
    ...
    check(all(a >= b - 1e-9 for a, b in zip(slots, slots[1:])),
          "expect_slot non-increasing in pick")
    check(all(abs(s - 0.10) < 1e-9 for s in slots[30:]),
          "round-2 picks share the flat 0.10 slot")

    print("mask honesty")
    if real:
        n_players = len(per_player)
        n_undrafted = sum(1 for prs in per_player.values()
                          if prs[0]["PED_UNDRAFTED"] == 1.0)
        total_players = doc["coverage"]["players_drafted"] + \
            doc["coverage"]["players_undrafted"] + \
            doc["coverage"]["players_unmatched_masked"]
        cov = (doc["coverage"]["players_drafted"] +
               doc["coverage"]["players_undrafted"]) / max(total_players, 1)
        check(cov >= 0.95, f"complete cache resolves >= 95% of players ({cov:.3f})")
        check(0.03 <= n_undrafted / max(n_players, 1) <= 0.45,
              f"undrafted share plausible ({n_undrafted}/{n_players})")
    else:
        check(doc["coverage"]["players_undrafted"] == 0,
              "partial cache labels NOBODY undrafted (masked instead)")
        check(doc["coverage"]["players_unmatched_masked"] > 0,
    ...
    if FAILURES:
        ...
        sys.exit(1)
    print("all pedigree gates passed"
          + ("" if real else " (fixture mode — run fetch_draft_history.py "
