...
def build_model(
    ...
    missing, _unexpected = model.load_state_dict(ckpt["model"], strict=False)
...
def jacobian_influence(
    ...
    fams = model.families
    n_t = len(fams)
    groups = [g for g in HEAD_GROUPS if g != "skills" or model.skill_towers is not None]
...
def write_feature_assets(
    ...
    doc = {
        ...
        "rows": int(n_rows),
        "method": (
            "Signed gradient x input, a_j = x_j * d(target)/d(x_j), over the "
        ...
        "maskedNote": (
            "A tower reads cat([x*m, m]), so a masked feature has exactly "
            ...
            "blocks": [
                {
                    ...
                    "dtype": "uint16",
                    "offset": 0,
                    "bytes": int(top_idx.nbytes),
                    "note": "index into `features`",
                },
                {
                    "name": "value",
                    "dtype": "float32",
                    "offset": int(top_idx.nbytes),
                    "bytes": int(top_val.nbytes),
                    "note": "signed contribution",
                },
            ],
        },
    }
