...
def main() -> None:
    ...
    for f in sorted(HERE.glob("data/gamelogs_*.jsonl")):
        ...
        for tid, roster in by_team.items():
            for (n1, pm1, team), (n2, pm2, _) in combinations(roster, 2):
                ...
                out_pairs.append({
                    "a": n1, "b": n2, "season": season, "team": team,
    ...
    for p in out_pairs:
        p["chemistry"] = round(0.5 * pctl(comps, p["complementarity"])
                               + 0.5 * pctl(pms, p["jointPM"]), 3)
    out_pairs.sort(key=lambda p: -p["chemistry"])

    (ASSETS / "chemistry.json").write_text(json.dumps({
        "method": ("pairs = same team-season, each >=1000 min, both "
