        ...
        if auto_resume:
            try:
                ...
                if ckpt_mtime < code_mtime:
                    print(
                        f"[ckpt] latest older than code ({ckpt_mtime} < {code_mtime}) — will NOT auto-resume unless --resume forced"
                    )
    ...
    class MultiTowerMTDeep(nn.Module):
        def __init__(self, use_era=False, use_attn=False, use_checkpoint=False):
            ...
            self.head_wins = nn.Sequential(
                nn.Linear(64, 32), nn.SiLU(), nn.Dropout(0.2), nn.Linear(32, 16), nn.SiLU(), nn.Linear(16, 1)
            )
...
def train_mt_long(args):
    ...
    X_draft_raw = np.array(
        [[d["inv"], d["log_o"], float(d["round"]), float(d["overall"]), d["draft_year_norm"]] for d in draft_data],
        dtype=np.float32,
    )
    ...
    tb_raw = np.array(
        [[d["avg_q"], float(d["seasons"]) / 5.0, float(d["overall"]) / 60.0, d["draft_year_norm"]] for d in draft_data],
        dtype=np.float32,
    )
    ...
    y_draft_mean = float(np.mean(y_qual))
    y_draft_std = float(np.std(y_qual)) if np.std(y_qual) > 1 else 1.0
    ...
    ckpt_mgr = CheckpointManager(
        version=f"v3_attn{int(use_attn)}_era{int(use_era)}_ema{int(args.ema)}", every=args.checkpoint_every
    )
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        if epoch % 10 == 0 or epoch == 1:
            print(
                f"[ep {epoch}/{args.epochs}] loss {epoch_loss_acc:.4f} best {best_loss:.4f} dMAE {draft_mae:.1f} wMAE {wins_mae:.2f} lr {optimizer.param_groups[0]['lr']:.2e} pat {pat}/{patience}"
            )
        ...
        if ckpt_mgr.should_save(epoch) or is_best:
            ckpt_mgr.save(
                ...
                epoch,
                epoch_loss_acc,
                {"draft_mae": draft_mae, "wins_mae": wins_mae},
                extra={"ema": bool(ema)},
            )
...
def run_zoo():
    ...
    zoo_results = {
        ...
        "meta": {
            ...
            "sklearn": SKLEARN,
            "torch": TORCH,
            "n_draft": len(draft_data),
            "n_fore": len(fore_data),
            "n_cap": len(cap_data),
        },
    }
    ...
    if SKLEARN and len(draft_data) > 20:
        ...
        models_reg = {
                ...
                [
                    ...
                    (
                        ...
                        RandomForestRegressor(
                            n_estimators=150, max_depth=12, min_samples_leaf=4, random_state=SEED, n_jobs=-1
                        ),
                    ),
                ]
            ),
            ...
            "HistGradientBoosting": HistGradientBoostingRegressor(
                random_state=SEED, max_iter=300, learning_rate=0.08, max_depth=8
            ),
        }
        try:
            import xgboost as xgb

            models_reg["XGBoost"] = Pipeline(
                [
                    ("scaler", StandardScaler()),
                    (
                        "reg",
        ...
        logreg_pipe = Pipeline(
            [
                ("scaler", StandardScaler()),
                ("clf", LogisticRegression(max_iter=800, random_state=SEED, class_weight="balanced")),
            ]
        )
        accs = []
        aucs = []
    ...
    if SKLEARN and cap_data and len(cap_data) > 5:
        ...
        rf = Pipeline(
            [("scaler", StandardScaler()), ("reg", RandomForestRegressor(n_estimators=100, random_state=SEED))]
        )
        rf.fit(Xc, yc)
...
def build_arg_parser():
    ...
    p.add_argument(
        "--checkpoint-mem", action="store_true", dest="checkpoint_mem", help="torch.utils.checkpoint for memory"
    )
