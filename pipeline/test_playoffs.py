...
def main() -> None:
    ...
    check(
        kawhi_min is not None and kawhi_min > 0,
    ...
    check(
        field("Kawhi Leonard", "2018-19", "PO_TEAM_WINS") == 16.0
        and field("Kawhi Leonard", "2018-19", "PO_ROUNDS") == 4.0,
        "champion Kawhi 2018-19: 16 wins / 4 rounds",
    )
    check(
        field("James Harden", "2018-19", "PO_ROUNDS") == 1.0,
    ...
    if real and ("Michael Jordan", "1997-98") in by:
        check(
            field("Michael Jordan", "1997-98", "PO_TEAM_WINS") == 15.0
        ...
        if ASSET.exists():
            ...
            check(
                len(series) == 4,
                f"Jordan 1997-98 series path length 4 (got {len(series)})",
            )
            if series:
                check(
                ...
                check(
                    mj.get("champion") is True or mj.get("rounds") == 4,
            ...
            for season in ("1996-97", "1997-98"):
                ...
                check(
                    ser and ser[-1].get("label") in ("Finals", "NBA Finals"),
                ...
                check(
                    not (ser and ser[-1].get("label") == "Conf finals"),
            ...
            if paths_asset.exists():
                ...
                check(
                    len(games) == 21,
                    f"Jordan 1997-98 game log 21 games (got {len(games)})",
                )
                if games:
                    check(
    ...
    if real:
        ...
        check(
            doc["coverage"]["appearances"] == len(rows) == 8,
