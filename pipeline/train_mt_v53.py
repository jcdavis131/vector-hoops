...
def load_foresight_dataset():
    ...
    try:
        sal_vals = [
            float(v.get("salary") or 0)
            for v in salaries.values()
            if isinstance(v, dict) and v.get("season") == "2024-25"
        ]
...
def get_era_ids(draft_year: int):
    ...
    if draft_year < 2002:
        cba_id = 0
    elif draft_year < 2011:
        cba_id = 1
    elif draft_year < 2023:
        cba_id = 2
    else:
        cba_id = 3
    ...
    if growth is None:
        ...
        if growth_val < 0:
            bucket = 0
        elif growth_val < 0.03:
            bucket = 1
        elif growth_val < 0.06:
            bucket = 2
    ...
    # TV id
    if draft_year <= 2015:
        tv_id = 0  # pre-2016 flat
    elif draft_year <= 2024:
        tv_id = 1  # 2016 $24B era
    else:
        tv_id = 2  # 2025 $76B
...
if __name__ == "__main__":
    ...
    if TORCH and len(draft_data) > 20:
        ...
        y_std = float(np.std(y_qual)) if np.std(y_qual) > 1 else 1.0
        ...
        mlp_results = {
            ...
            "arch": "14->256 LN SiLU d0.35 ->128 LN SiLU d0.35 ->64 LN SiLU d0.3 ->32 SiLU ->1 era14 200ep cosAnneal AdamW lr1e-3 wd2e-4 pat20",
        }
    ...
    if TORCH and len(draft_data) > 20 and fore_data and len(cap_data) >= 5:
        ...
        tb_raw = np.array(
            [
                [d["avg_q"], float(d["seasons"]) / 5.0, float(d["overall"]) / 60.0, d["draft_year_norm"]]
                for d in draft_data
            ],
            dtype=np.float32,
        )
        ...
        tb_f_raw = (
            np.array(
                [[d["tm"] / 2000, d["gp"] / 82, d["surplus"] / 1e6, d["contract_age"] / 5] for d in fore_data],
                dtype=np.float32,
            )
            if fore_data
            else np.zeros((0, 4))
        ...
        tc_f_raw = (
            np.array(
                [
                    [
                        float(d["contract_age"]) / 5,
                        float(d["cap_growth_proxy"] * 10),
                        float(d["salary_growth_proxy"] * 10),
                        float(d["maturation_ratio"]),
                    ]
                    for d in fore_data
                ],
                dtype=np.float32,
        ...
        class MultiTowerV3(nn.Module):
            def __init__(self):
                ...
                self.era_emb = EraEmbed(
                    out_dim=4
                )  # learned league embedding, concat already in C but also direct? we will keep direct bypass used in C only via raw era; emb module used for cba id path when we call separate
                self.shared = SharedTrunkV3()
                self.head_draft = nn.Sequential(
                    nn.Linear(32, 64), nn.SiLU(), nn.Linear(64, 32), nn.SiLU(), nn.Linear(32, 1)
                )
                self.head_bust = nn.Sequential(nn.Linear(32, 16), nn.SiLU(), nn.Linear(16, 1))
                self.head_fore = nn.Sequential(
                    nn.Linear(32, 64), nn.SiLU(), nn.Linear(64, 32), nn.SiLU(), nn.Linear(32, 1)
                )
                self.head_wins = nn.Sequential(
                    nn.Linear(32, 64),
                    nn.SiLU(),
                    nn.Linear(64, 32),
                    nn.SiLU(),
        ...
        def train_one(model_cls, name, epochs=280, pat=22):
            ...
            res = {
                ...
                "draft_mae": ev_draft["mae"],
                "draft_r2": ev_draft["r2"],
                "draft_rmse": ev_draft["rmse"],
                "wins_mae": ev_wins["mae"],
                "wins_r2": ev_wins["r2"],
                "wins_rmse": ev_wins["rmse"],
                "foresight_mae": ev_fore.get("mae"),
                "foresight_r2": ev_fore.get("r2"),
            }
            print(
                f"{name} loss {res['loss_final']} draft {res['draft_mae']} r2 {res['draft_r2']} wins {res['wins_mae']} r2 {res['wins_r2']} ep {res['early_stop_epoch']}"
            )
            return res, mt
        ...
        class MultiTowerV4(nn.Module):
            ...
            def forward(self, ta, tb, tc, td):
                ...
                h = F.silu(self.ln1(self.shared_fc1(shared_in)))
                h = self.do(h)
                h = F.silu(self.ln2(self.fc2(h)))
                h = self.do(h)
                h = F.silu(self.ln3(self.fc3(h)))
                return {
                    "draft": self.head_draft(h).squeeze(-1),
                    "bust": self.head_bust(h).squeeze(-1),
                    "foresight": self.head_fore(h).squeeze(-1),
                    "wins": self.head_wins(h).squeeze(-1),
                }

        res_v4, model_v4 = train_one(MultiTowerV4, "MTv4_attn", epochs=300, pat=25)

        # Save into base_results
        base_results["multi_tower_multitask_v3"] = {
            "arch": "TowerA/B/C/D 2-layer 64->32 LN SiLU d0.3 shared 128->128->64->32 residual LN skip 128->32 heads deeper draft 64->32->1 wins 64->32->16->1 era_emb 4dim concat to C timing 8dim lr8e-4 cos warmup10 wd2e-4 pat22 epochs280",
            **res_v3,
        }
        base_results["multi_tower_multitask_v4"] = {
            "arch": "4 towers 32 + MultiHeadAttention 4 heads 16kd scaled dot 4 tokens attended LN residual gated sum 128->64 tanh*sigmoid trunk 64->128->64->32 heads same era_emb",
            **res_v4,
        }
        base_results.setdefault("meta", {})["hill_climb_v5_3"] = {
            "mt_v3": res_v3,
            "mt_v4": res_v4,
            "era_features": "cba_id 0-3 bucket 0-4 tv 0-2 growth float, embed 4dim learned via small MLP concat to TowerC",
        }
