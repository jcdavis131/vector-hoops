...
def snapshot() -> dict:
    ...
    return {
        ...
        "n_seasons": len(set(p["season"] for p in vec["players"])),
...
def main() -> None:
    ...
    ap.add_argument("--offline", action="store_true",
                    help="skip the network fetch entirely")
    ...
    steps.append(run_step(
        "rebuild skills",
        [sys.executable, "pipeline/build_skills.py"], required=True))
    steps.append(run_step(
        "skill gates",
    ...
    if prev:
        ...
        print(f"growth since last run: {d_rows:+d} player-seasons; "
