...
def v2_clusters(data: dict) -> None:
    ...
    for p in data["players"]:
        best = min(
            cents,
            key=lambda c: sum((a - b) ** 2 for a, b in zip(p["v"], cents[c], strict=False)),
        )
        if best != p["c"]:
            mism += 1
...
def v3_deadline() -> None:
    ...
    for f in HERE.glob("data/gamelogs_*.jsonl"):
        ...
        logs_by_season[season] = [json.loads(line) for line in f.read_text(encoding="utf-8").splitlines()]
    checked = 0
    for m in quiz:
        ...
        if len(before) != m["gBefore"] or len(after) != m["gAfter"]:
            fail(f"{m['name']} game counts {len(before)}/{len(after)} vs {m['gBefore']}/{m['gAfter']}")
        checked += 1
...
def v5_procrustes(data: dict) -> None:
        ...
        for i in range(n):
            for j in range(n):
                ...
                if abs(dot - want) > 5e-3:
                    fail(f"{season}: chained transform not orthogonal ({i},{j})={dot:.4f}")
                    break
            else:
                continue
            break
    ...
    if len(dj["chainedToRoot"]) != len(seasons):
        fail(f"chainedToRoot covers {len(dj['chainedToRoot'])} of {len(seasons)} seasons")
    print(f"  {len(dj['pairs'])} pairs, {len(dj['chainedToRoot'])} chained transforms, all orthogonal")
...
def v9_wide_skills(data: dict) -> None:
    ...
    print(f"  {len(grades)} wide grades ({len(skills)} skills); key hit {hits}/{len(data['players'])} vector rows")
...
def v13_mtnn_jacobian(data: dict) -> None:
    ...
    if arch_path.exists():
        ...
        if af and jf and af != jf:
            fail(
                f"jacobian towerFamilies != arch (stale export): jac-only={sorted(jf - af)} arch-only={sorted(af - jf)}"
            )
        if jac.get("dEmb") is not None and arch.get("dEmb") is not None and jac["dEmb"] != arch["dEmb"]:
    ...
    print(f"  jacobian: {shape[0]}×{shape[1]}×{shape[2]} ({nbytes // 1024} KB), targets={jac.get('targets')}")
...
def v13b_mtnn_attribution(data: dict) -> None:
    ...
    if nbytes != expected:
        ...
        if feats and int(idx.max()) >= len(feats):
            fail(f"attribution index {idx.max()} out of range for {len(feats)} features")
    ...
    leaked = [f for f in feats if cov.get(f) == 0.0 and any(abs(pop_abs.get(t, {}).get(f, 0.0)) > 0.0 for t in pop_abs)]
    if leaked:
...
def v15_season_norms(data: dict) -> None:
    ...
    print(f"  {checked} (season, feature) pairs invert cleanly; {len(shrunk)} shrunk features correctly withheld")
...
def v16_draft_board(data: dict) -> None:
    ...
    no_expect = [n for n, p in players.items() if n in charted and p.get("undrafted") and p.get("expect_slot") is None]
    if no_expect:
        fail(f"{len(no_expect)} undrafted players lack expect_slot (e.g. {no_expect[0]})")

    leaked_pick = [
        n for n, p in players.items() if n in charted and p.get("undrafted") and p.get("overall") is not None
    ]
    if leaked_pick:
        fail(f"{len(leaked_pick)} undrafted players carry a pick number (e.g. {leaked_pick[0]})")

    at_61 = [n for n, p in players.items() if n in charted and not p.get("undrafted") and p.get("overall") == 61]
    if at_61:
    ...
    print(
        f"  {undrafted} charted undrafted players eligible as steals; max real pick {max_pick}; sentinel 61 unoccupied"
    )
...
def v14_stated_limitations(data: dict) -> None:
    ...
    if not train.exists() or not manifest_p.exists():
        ...
        if leaked > 1e-4:
            fail(
                f"imputation under mask: |Z*(1-M)| = {leaked:.6f} (must be 0) — methods.html claims masked, not imputed"
            )
        else:
