...
def fetch_html(end_year: int) -> str:
    ...
    try:
        html = subprocess.check_output(
            ["curl", "-sL", "-A", UA, "--compressed", "-m", "20", url], text=True, stderr=subprocess.DEVNULL
        )
        if len(html) > 5000:
            return html
    except Exception as e:
...
def parse_wins_ou(html: str) -> dict:
    ...
    # Pattern1: wins_ou
    for abbr, ou in re.findall(
        r'/teams/([A-Z]{3})/\d+\.html.*?</a>.*?<td[^>]*data-stat="(?:wins_ou|over_under)"[^>]*>([0-9]{2,3}\.[05])</td>',
        clean,
        re.S,
    ):
        try:
